...
LINK="$HOME/.dolphin-emu"
# Uses unofficial AppImage by pkgforge-dev which tracks official Dolphin releases
API_URL="https://api.github.com/repos/pkgforge-dev/Dolphin-emu-AppImage/releases"
ICON_URL="https://raw.githubusercontent.com/dolphin-emu/dolphin/master/Data/dolphin-emu.svg"
DESKTOP_FILE="$DESKTOP_DIR/dolphin.desktop"

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "           Dolphin Setup"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "ℹ️  Using unofficial AppImage by pkgforge-dev"
echo "   (tracks official Dolphin releases)"
echo ""
...
# Skip pre-releases (e.g. nightly tag) and find the latest stable
...
for r in releases:
    ...
    for a in r.get('assets', []):
        name = a['name'].lower()
        if name.endswith('.appimage') and 'x86_64' in name and 'zsync' not in name:
...
if [ "$NEEDS_DOWNLOAD" = true ]; then
    ...
    # Remove old AppImage
    find "$INSTALL_DIR" -maxdepth 1 -type f -name "Dolphin*.AppImage" -delete
    ...
    APPIMAGE_NAME=$(find "$INSTALL_DIR" -maxdepth 1 -type f -name "Dolphin*.AppImage" | head -n1 | xargs basename)
fi

# ── Step 4: Verify AppImage exists ───────────────────────────────────────────
if [ -z "$APPIMAGE_NAME" ]; then
    echo "❌ Could not find Dolphin AppImage in $INSTALL_DIR."
    sleep 10
    exit 1
fi
...
mkdir -p "$ICON_DIR"
curl -L "$ICON_URL" -o "$ICON_DIR/dolphin.svg"
...
rm -f "$DESKTOP_DIR"/dolphin*.desktop
rm -f "$DESKTOP_DIR"/Dolphin*.desktop

cat > "$DESKTOP_FILE" << EOF
[Desktop Entry]
Name=Dolphin
Comment=GameCube / Wii Emulator
Exec=$INSTALL_DIR/$APPIMAGE_NAME
Icon=$ICON_DIR/dolphin.svg
Type=Application
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if [ "$NEEDS_DOWNLOAD" = true ]; then
    echo "   🎮 Dolphin $LATEST_VERSION installed!"
else
    echo "   🎮 Dolphin is already up to date!"
fi
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
