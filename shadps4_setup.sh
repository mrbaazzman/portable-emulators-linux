...
LINK="$HOME/.local/share/shadPS4"
API_URL="https://api.github.com/repos/shadps4-emu/shadps4-qtlauncher/releases"
ICON_URL="https://raw.githubusercontent.com/shadps4-emu/shadps4-qtlauncher/main/src/images/net.shadps4.shadPS4.svg"
DESKTOP_FILE="$DESKTOP_DIR/shadps4.desktop"

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "       shadPS4 Qt Launcher Setup"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
if [ "$NEEDS_DOWNLOAD" = true ]; then
    ZIP_NAME=$(basename "$DOWNLOAD_URL" | cut -d'?' -f1)
    echo ""
    echo "📥 Downloading $ZIP_NAME..."

    # Remove old executables
    find "$INSTALL_DIR" -maxdepth 1 -type f -name "shadPS4*" \
        ! -name "*.svg" ! -name "*.png" ! -name "*.desktop" ! -name "*.sh" ! -name "version.txt" \
        -delete

    curl -L "$DOWNLOAD_URL" -o "$INSTALL_DIR/$ZIP_NAME"
...
# ── Step 4: Auto-detect the executable ───────────────────────────────────────
EXEC_NAME=$(find "$INSTALL_DIR" -maxdepth 1 -type f -name "shadPS4*" \
    ! -name "*.svg" ! -name "*.png" ! -name "*.desktop" ! -name "*.sh" ! -name "*.txt" \
    | head -n1 | xargs basename)

if [ -z "$EXEC_NAME" ]; then
    echo "❌ Could not find shadPS4 executable in $INSTALL_DIR."
    sleep 10
    exit 1
fi
...
mkdir -p "$ICON_DIR"
curl -L "$ICON_URL" -o "$ICON_DIR/shadps4.svg"
...
rm -f "$DESKTOP_DIR"/shadps4*.desktop
rm -f "$DESKTOP_DIR"/shadPS4*.desktop
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if [ "$NEEDS_DOWNLOAD" = true ]; then
    echo "   🎮 shadPS4 $LATEST_VERSION installed!"
else
    echo "   🎮 shadPS4 is already up to date!"
fi
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
