...
LINK="$HOME/.config/ppsspp"
API_URL="https://api.github.com/repos/hrydgard/ppsspp/releases/latest"
ICON_URL="https://upload.wikimedia.org/wikipedia/commons/d/dc/PPSSPP_logo.svg"
DESKTOP_FILE="$DESKTOP_DIR/ppsspp.desktop"
...
for a in release.get('assets', []):
    name = a['name'].lower()
    if name.endswith('.appimage') and 'anylinux' in name and 'x86_64' in name:
...
if [ "$NEEDS_DOWNLOAD" = true ]; then
    ...
    # Remove old AppImage
    find "$INSTALL_DIR" -maxdepth 1 -type f -name "PPSSPP*.AppImage" -delete
    ...
    APPIMAGE_NAME=$(find "$INSTALL_DIR" -maxdepth 1 -type f -name "PPSSPP*.AppImage" | head -n1 | xargs basename)
fi

# ── Step 4: Verify AppImage exists ───────────────────────────────────────────
if [ -z "$APPIMAGE_NAME" ]; then
    echo "❌ Could not find PPSSPP AppImage in $INSTALL_DIR."
    sleep 10
    exit 1
fi
...
mkdir -p "$ICON_DIR"
curl -L "$ICON_URL" -o "$ICON_DIR/ppsspp.svg"
...
rm -f "$DESKTOP_DIR"/ppsspp*.desktop
rm -f "$DESKTOP_DIR"/PPSSPP*.desktop
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if [ "$NEEDS_DOWNLOAD" = true ]; then
    echo "   🎮 PPSSPP $LATEST_VERSION installed!"
else
    echo "   🎮 PPSSPP is already up to date!"
fi
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
